...
# Run 9 — dispersed-learnings fixture, 2026-09-16 (author-skills gains "mine the transcript"; adversarial-review gains model fallback; never-block-on-the-human strengthened)

Motivation: a comparison audit against a sibling pstack-derived suite (`pathfinder`'s `tallboy-mode`) found two of its skills — `interrogate` (multi-model adversarial review) and `reflect` (three-lens transcript mining, routed to a skill edit) — structurally stronger in specific ways than plays' `adversarial-review` and `author-skills`. Backfilling the real, non-redundant parts.

**adversarial-review (mechanical, no eval — same tier as the security-scan fallback line):** added `interrogate`'s model-slug fallback (check the spawn error for valid slugs, prefer the same family's highest reasoning tier, don't block the review on a mismatch). Note for the record, not acted on: this comparison also found `adversarial-review` already has real content `interrogate` lacks — five calibration guards against review noise, and dual lens+model diversity where `interrogate` only diversifies on model. The backfill here is genuinely two-way, not a one-way port.

**author-skills "mine the transcript" (real behavioral addition, dogfooded):** ported `reflect`'s three-lens (judgment/tooling/divergent) transcript-mining structure as a companion to baseline-first, for sessions with dispersed learnings and no single clean failure to reproduce. Fixture: a synthetic transcript with three threads — a test-scope correction, a cursor-pagination dead-end that generalized into a rule ("we've been bitten by this before"), and a fixture-helper extraction.

**Baseline** (current author-skills, no mining section): correctly caught the recurring-correction thread via existing baseline-first reasoning, wrote it to `OBSERVED-FAILURES.md` per the existing remember mechanism, and correctly declined to fabricate new principle text from a single anecdote — good, disciplined behavior. But it never systematically surveyed the other two threads, and specifically missed that the transcript's agent let a live defect in a sibling endpoint (surfaced only by the human's aside, not investigated directly) evaporate into a summary sentence with nothing filed.

**Candidate, real result:** applying the three lenses across all three threads, correctly *Rejected* two (already covered by existing principles — no gap manufactured to fill) and *Accepted* exactly one genuinely new finding: the live-defect-surfaced-by-aside gap baseline missed, which nothing in the existing suite clearly covers. Correctly *Backlogged* (not Accepted) a lower-priority item. This is real, non-redundant value — the multi-lens survey caught something the reactive-only baseline structurally couldn't.

**Process incident, itself real evidence:** the first candidate run, given the open-ended framing "use author-skills to capture anything worth improving," autonomously abandoned the scoped task and spun up its own nested eval pipeline — two new fixture repos, its own sub-agents, its own injected corrections — chasing a different, self-selected gap, burning roughly a quarter million tokens on an unplanned, unbounded tangent before being redirected. Two orphaned grandchild agents from that tangent independently behaved well on their own axis (declined to expand scope on an unverified claim, filed a backlog issue instead) — but the incident itself is the finding: an agent given license to "consider promoting" read `author-skills`' own "prove it blind before promoting" line as authorization to self-direct a full nested investigation. **Change made in response:** the mining section now explicitly stops at the Accepted/Rejected/Backlog synthesis — drafting only, no autonomous sub-agent eval pipeline, promotion is a separate, deliberate decision for whoever reviews the list. Not re-tested after this addition (see caveats) given the token cost already spent; the next real invocation of this section is the actual test of whether the stop-boundary holds.
