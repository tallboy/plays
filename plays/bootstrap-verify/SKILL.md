...
- **Evidence:** what to capture for a proof and where it goes. Check whether that location is tracked: scratch and artifact directories are usually gitignored, so a screenshot path cited in a committed file points at one machine's local run and rots with nothing to catch it. Cite what travels with the repo — the spec, the assertion, the test — and treat captured images as evidence of a run, not as the map's proof. Exercise the real user path, not internal setters or test-only endpoints; capture the action and the resulting state, not just the final screen; verify side effects (files written, rows inserted) alongside what's visible. When the safe path is a dry-run mode, verify what it actually skips by observing rather than trusting its name.
