...
**Handoff fork (pipeline mode).** When your instructions say the pipeline owns delivery — you may not push, merge, or open PRs — Ship changes shape: keep the commit discipline below, write the PR description (sections below, first line `Closes #N`) to `.karakuri-pr.md` at the worktree root, and stop. No branch creation, no push, no PR. The pipeline validates the file and delivers it.
...
**Description.** First line: `Closes #N` when an issue exists — before any heading; automation parses it there. Then these sections in order, dropping any that would be empty:
...
Attach screenshots or recordings when they prove a claim. No `## Summary` or `## Test plan` boilerplate; a commit body doesn't restate its subject.
