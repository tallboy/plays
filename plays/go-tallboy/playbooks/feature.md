...
6. Test the new behavior. New behavior gets a test that would fail without it. Before writing each test, name the production change that would break it — a test with no such change is asserting on scaffolding, not behavior.
