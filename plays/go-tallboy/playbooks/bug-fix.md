...
4. Fix the cause, as small as the confirmed mechanism justifies. If the fix crosses a design boundary, run the arena skill first. Three refuted fixes in a row means the flaw is in your model of the system, not the patch — stop patching and re-derive the mechanism before attempting a fourth.
5. Write a regression test that fails before the fix and passes after. Run the red leg for real — with the fix stashed or reverted — and capture the failing output; asserting it "would fail" is not evidence. A test that passes before the fix is testing something else. Skip only with a stated reason (`skip: <fact about this task>`).
